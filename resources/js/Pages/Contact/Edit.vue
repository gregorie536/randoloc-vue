            ...
            <form @submit.prevent="submitForm" class="space-y-6">
                ...
                <div v-for="(contact, index) in contacts" :key="contact.id" class="border-b-2 border-aliceblue pb-6 mb-6">
                    ...
                    <div class="flex flex-col mb-4">
                        ...
                        <input :id="'lastname' + index" v-model="contact.lastname" class="p-2 rounded-md border border-aliceblue focus:outline-none focus:border-nav-bg-color" />
                    </div>
                    <div class="flex flex-col mb-4">
                        <label :for="'firstname' + index" class="text-main-text-color mb-1">Prénom :</label>
                        <input :id="'firstname' + index" v-model="contact.firstname" class="p-2 rounded-md border border-aliceblue focus:outline-none focus:border-nav-bg-color" />
                    </div>
                    <div class="flex flex-col mb-4">
                ...
                <button type="button" class="bg-gray-300 text-black py-2 px-6 rounded-md hover:bg-gray-400 focus:outline-none" @click="goToDashboard">Annuler</button>
            </form>
        </div>
    </AuthenticatedLayout>
</template>
